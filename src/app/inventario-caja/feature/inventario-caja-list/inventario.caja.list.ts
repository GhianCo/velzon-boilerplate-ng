...
@Component({
  standalone: true,
  templateUrl: 'inventario.caja.list.html',
...
export class InventarioCajaList {
  ...
  // Variable para la caja seleccionada en el modal
  selectedCajaId: number = -1;
  ...
  constructor(private modalService: NgbModal,
              ...
              public inventarioCajaStore: InventarioCajaStore,
    ...
    this.dateRangeControl.valueChanges.subscribe((value: any) => {
      ...
      if (dates.length === 2) {
        ...
        this.inventarioCajaStore.setDateRange([dates[0], dates[1]]);
      } else if (dates.length === 1) {
        // Si solo hay una fecha, usar el mismo día como inicio y fin
        this.inventarioCajaStore.setDateRange([dates[0], dates[0]]);
      }
    });

    effect(() => {
      const data: any = this.inventarioCajaStore.vm().inventarioCajaData;
    ...
    // Sincronizar selectedCajaId con el store
    effect(() => {
      const cajaId = this.inventarioCajaStore.vm().filtersToApply?.caja_id;
      if (cajaId !== undefined) {
  ...
  private configureFlatpickr() {
    this.flatpickrOptions = {
      mode: 'range',
      dateFormat: 'd/m/Y',
      defaultDate: this.inventarioCajaStore.getDateRangeForComponent(),
      locale: {
        firstDayOfWeek: 1,
        weekdays: {
      ...
      onChange: (selectedDates: Date[], dateStr: string, instance: any) => {
        if (selectedDates && selectedDates.length >= 1) {
          if (selectedDates.length === 1) {
            ...
            this.inventarioCajaStore.setDateRange([selectedDates[0], selectedDates[0]]);
            ...
            this.inventarioCajaStore.setDateRange([selectedDates[0], selectedDates[1]]);
          }
        }
      }
    };
  ...
  changePage(pageNumber: number) {
    this.inventarioCajaStore.changePagination(pageNumber)
  }
  ...
  /**
   * Intenta aperturar una caja, validando primero que no haya una caja abierta
   * Delega toda la lógica al servicio centralizado
   */
  onAperturarCaja() {
    this.aperturaCajaValidator.intentarAperturar();
  }

  cerrarCaja(id: any) {
    this.router.navigate(['/inventario-caja/cerrar/' + id])
  }

  replicarInventario(id: any) {
    this.router.navigate(['/inventario-caja/replicar/' + id])
  }

  visualizarInventario(id: any) {
    this.router.navigate(['/inventario-caja/visualizar/' + id])
  }

  /**
   * Descarga el PDF del resumen de operación de caja
   */
  descargarPdfInventario(id: any) {
    // Obtener los datos del resumen
    this.inventarioRemoteReq.requestResumenOperacionCaja(id).subscribe({
      next: (response: any) => {
        if (response?.data) {
          this.inventarioCajaPdfService.generarPdfResumen(response.data);
        }
      },
      error: (error) => {
  ...
  openFilters(content: TemplateRef<any>) {
    ...
    const currentFilters = this.inventarioCajaStore.vm().filtersToApply;
    this.originalFilters = {
      caja_id: currentFilters?.caja_id ?? -1,
    ...
    const currentCajaId = currentFilters?.caja_id;
    this.selectedCajaId = currentCajaId !== undefined ? currentCajaId : -1;
  ...
  aplicarFiltros() {
    ...
    this.inventarioCajaStore.applyFilters();
  ...
  cancelarFiltros() {
    ...
    if (this.originalFilters) {
      this.inventarioCajaStore.setFilters({
        caja_id: this.originalFilters.caja_id,
      ...
      if (this.originalFilters.startDate && this.originalFilters.endDate) {
        this.inventarioCajaStore.setDateRange([
      ...
      // Restaurar la caja seleccionada en el select
      this.selectedCajaId = this.originalFilters.caja_id;
    }
  ...
  limpiarFiltros() {
    ...
    this.inventarioCajaStore.clearFilters();

    // Resetear la caja seleccionada
    this.selectedCajaId = -1;

    // Actualizar el FormControl con las nuevas fechas
    const newDateRange = this.inventarioCajaStore.getDateRangeForComponent();
  ...
  // Método para cambiar caja desde el modal (NO aplica filtros automáticamente)
  onCajaChangeFromModal(event: any) {
    const value = event?.value;
    const cajaId = value === 'null' || value === '' ? -1 : parseInt(value);
    this.inventarioCajaStore.setFilters({ caja_id: cajaId });
    // NO aplicar filtros aquí - solo actualizar el estado
  }

  // Método para remover el badge de caja (SÍ aplica filtros automáticamente)
  onCajaRemove() {
    this.inventarioCajaStore.setFilters({ caja_id: -1 });
  ...
  onSalaChange(event: any) {
    ...
    this.inventarioCajaStore.setFilters({ sala_id: salaId });
    // NO aplicar filtros aquí - solo actualizar el estado
  }
