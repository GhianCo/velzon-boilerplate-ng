...
/**
 * Componente para visualizar el resumen de una operación de caja
 * Muestra 3 tablas: Inventario Apertura, Inventario Cierre y Suma Diaria
 */
@Component({
  selector: 'app-inventario-caja-visualizar',
  templateUrl: './inventario.caja.visualizar.html',
  standalone: true,
  imports: [
...
export class InventarioCajaVisualizar implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.route.params.subscribe(params => {
      ...
      if (this.operacionCajaId) {
        this.loadResumen(this.operacionCajaId);
      }
    });
  }

  /**
   * Cargar el resumen de la operación de caja a través del store
   */
  loadResumen(operacionCajaId: string) {
    this.inventarioCajaStore.loadResumenOperacionCaja(operacionCajaId).subscribe();
  }
   ...
   */
  get resumenData() {
    return this.inventarioCajaStore.vm().resumenOperacionData;
  }
   ...
   */
  get isLoading() {
    return this.inventarioCajaStore.vm().resumenOperacionLoading;
  }
   ...
   */
  get hasError() {
    return !!this.inventarioCajaStore.vm().resumenOperacionError;
  }
